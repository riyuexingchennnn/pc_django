...
class SystemMonitorAdmin(admin.AdminSite):
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "system-monitor/",
                self.admin_view(self.system_monitor),
                name="system-monitor",
            ),
        ]
    ...
    def system_monitor(self, request):
        ...
        memory_pie_sizes = [
            ...
            memory_free / memory_total * 100,
        ]

        axs[2].pie(
            memory_pie_sizes,
            labels=memory_pie_labels,
            autopct="%1.1f%%",
            startangle=90,
            colors=colors,
        )
        ...
        # 渲染模板
        return render(
            request, "admin/system_monitor.html", {"image_base64": image_base64}
        )
