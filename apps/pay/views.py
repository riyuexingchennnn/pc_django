...
def alipay(request):
    ...
    try:
        ...
        request_data.return_url = "http://localhost:8000/pay/alipay/return"  # 支付完成后的跳转页面
        request_data.notify_url = "http://localhost:8000/pay/alipay/notify"  # 异步通知地址
            
        # 调用支付宝接口
        response_content = client.page_execute(request_data, http_method="GET")
        return JsonResponse({'payment_url': response_content})
